...
export const GetAllTheaterSystem = (callback) => {
  return (dispatch) => {
    TheaterSystem.getAllTheaterSystem()
      .then((res) => {
...
export const getALLGroupTheatherWithIdTheatherSystem = (
  idTheaterSystem,
  callback
) => {
  return (dispatch) => {
    TheaterSystem.getALLGroupTheatherWithIdTheatherSystem(idTheaterSystem)
      .then((res) => {
...
export const getALLInfoFollowTheaterSystem = (idTheaterSystem, callback) => {
  return (dispatch) => {
    TheaterSystem.getALLInfoFollowTheaterSystem(idTheaterSystem)
      .then((res) => {
        ...
        console.log("phim", res.data[0].listCumRap[0].danhSachPhim);
        dispatch(
          createAction(
            SET_DATA_LIST_MOVIE_WITH_THEATER,
            res.data[0].listCumRap[0].danhSachPhim
          )
        );
        callback();
      })
